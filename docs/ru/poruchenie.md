...
Просмотр поручений, а также их добавление возможно только с главной страницы. Подробное описание отображения и разбивки по карточкам приведено в разделе «Дополнительно».
...
При создании нового поручения необходимо включить флажок переноса, как показано на скриншоте ниже, и выбрать из списка номер того поручения, чей срок истекает. Дальнейшие шаги аналогичны созданию поручения. 
...
**Если по завершающемуся поручению есть отрицательный остаток (сумма в минусе),** то при создании поручения, на которое переносятся остатки, нельзя указать бюджет меньше суммы отрицательного остатка. 

{% endnote %}

### Вариант № 2

Продлить поручение можно при просмотре информации о поручении по кнопке «Продлить поручение»: после этого также произойдёт переход к экрану создания поручения — так же, как в варианте 1. Однако, флажок переноса уже будет включён, а номер поручения предвыбран в выпадающем списке. 
